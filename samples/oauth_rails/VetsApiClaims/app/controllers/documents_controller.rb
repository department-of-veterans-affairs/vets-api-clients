class DocumentsController < ApplicationController
  ...
  def create
    res = RestClient.post("#{Figaro.env.vets_api_url}/services/vba_documents/v1/uploads", nil,  apikey: Figaro.env.va_api_key)
  ...
  def update_status
    document = Document.find params[:id]
    res = RestClient.get("#{Figaro.env.vets_api_url}/services/vba_documents/v1/uploads/#{document.guid}",  apikey: Figaro.env.va_api_key)
    parsed = JSON.parse(res.body)['data']
    document.update(
      status: parsed['attributes']['status'],
